...
// Redirect malloc/free/realloc to mimalloc within this compilation unit.
// memory::allocate() and memory::deallocate() are the central allocation
// bottleneck — all Z3 allocations flow through them. The mimalloc-override
// header redefines malloc→mi_malloc etc. via macros, so these functions
// transparently use mimalloc's fast allocator.
// Must come AFTER platform <malloc.h> includes to avoid type conflicts.
#ifdef Z3_USE_MIMALLOC
